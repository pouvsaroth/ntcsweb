...
<template>
  <div class="max-w-4xl">
    <div class="mb-6">
      ...
      <p class="mt-1 text-sm text-neutral-500">{{ t('admin.students.pageSubtitle') }}</p>
    </div>
    ...
    <form v-else class="space-y-10" @submit.prevent="submit">
      ...
      <section>
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-2">
          <BaseInput v-model="form.student_code" required :label="t('admin.students.studentCode')" :error="errors.student_code?.[0]" />
          ...
          <BaseInput v-model="form.phone" :label="t('admin.students.phone')" :error="errors.phone?.[0]" />
