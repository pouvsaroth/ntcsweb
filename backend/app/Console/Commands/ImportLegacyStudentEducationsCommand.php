...
/**
 * Step 2 of the legacy student-education migration: load the CSV that
 * scripts/legacy-import/export-legacy-student-educations.php produced into
...
{
    protected $signature = 'students:import-legacy-educations
        {file : Path to the CSV produced by export-legacy-student-educations.php}
        {--tenant= : Numeric ID of the tenant these education records belong to}
        {--dry-run : Parse and validate the file without writing anything}
        {--force : Import even if this tenant already has student_educations rows}';

    protected $description = 'Import student education history from a legacy t_school_student_education CSV export for one tenant';

    private const CHUNK_SIZE = 500;

    private const MAX_LISTED_ERRORS = 50;

    /** @var array<string, string> CSV header (lowercased) => student_educations column */
    ...
    private function importRows($handle, int $tenantId, bool $dryRun): int
    {
        ...
        while (($line = fgetcsv($handle)) !== false) {
            ...
            if ($code === '' || $schoolName === '' || $address === '' || $skill === '' || $detail === '' || $startDate === null) {
                $skippedCount++;
                $this->recordError($errors, $rowNumber, 'Missing required StudentID, SchoolName, Address, Skill, Detail, or a valid StartDate.');
    ...
    /**
     * @param  resource  $handle
     * @return array<int, string>|null column index => student_educations field name
     */
