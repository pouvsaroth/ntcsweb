...
class AttendanceTest extends TestCase
{
    ...
    /**
     * Recording attendance is gated purely by holding attendance.create/
     * update — not by being personally assigned to teach that class. A
     * school wants any teacher or assistant able to cover any class (a
     * substitute, a shared room, someone helping out) — see
     * SchoolClassPolicy::recordAttendance(). Covers a teacher assigned to a
     * *different* class, one assigned to no class at all, and a class with
     * no teacher assigned yet — all three used to be blocked, none are now.
     */
    public function test_any_attendance_permission_holder_can_take_attendance_for_any_class_regardless_of_teaching_assignment(): void
    {
        ...
        // Assigned to a *different* teacher — exercises "someone else's class."
        ...
        // No teacher assigned to this one at all.
        [$unassignedClass, $unassignedEnrollments] = $this->classWithStudents(1);
        $this->assertTrue($unassignedClass->teachingStaff()->doesntExist());

        foreach ([
            [$ownClass, $ownEnrollment],
            [$otherClass, $otherEnrollment],
            [$unassignedClass, $unassignedEnrollments[0]],
        ] as [$class, $enrollment]) {
            $this->postJson("/api/v1/classes/{$class->id}/attendance", [
                'date' => now()->toDateString(),
